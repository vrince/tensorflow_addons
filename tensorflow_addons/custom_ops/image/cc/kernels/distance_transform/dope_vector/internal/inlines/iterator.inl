...
#include "tensorflow_addons/custom_ops/image/cc/kernels/distance_transform/dope_vector/internal/iterator.hpp"
#include <limits>
...
inline Iterator<T, Dimension, Const>::Iterator()
	: _currentIndex(IndexD::Zero()), _valid(false)
{
}

template <typename T, SizeType Dimension, bool Const>
inline Iterator<T, Dimension, Const>::Iterator(DopeVectorType &dope_vector, const SizeType i, const bool valid)
	: _data(dope_vector), _currentIndex(valid ? dope::to_index<Dimension>(i, dope_vector.allSizes()) : IndexD::Zero()), _valid(valid)
{
...
inline Iterator<T, Dimension, Const>::Iterator(DopeVectorType &dope_vector, const IndexD &index, const bool valid)
	: _data(dope_vector), _currentIndex(valid ? index : IndexD::Zero()), _valid(valid)
{
	if (_valid)
	{
